...
public class StudentProjectService {
    ...
    public ResponseEntity<List<Project>> getVisibleProjects() {
        try {
            ...
            // responseEntity=restTemplate.exchange(FACULTY_SERVICE+"/"+"visible",
            // HttpMethod.GET,
            // null,
    ...
    public ResponseEntity<String> updateProjectPrefernce(int studentId, int projectId, int newPreference) {
        try {
            ...
            StudentProject targetProject = appliedProjects.stream().filter(p -> p.getProjectId() == projectId)
    ...
    public ResponseEntity<List<StudentProject>> findProjectByProjIdAndStudIds(int projectId, List<Integer> studentIds) {
        try {
            List<StudentProject> studentProjects = studentProjectDao.findByProjectIdAndStudent_StudentIdIn(projectId,
                    studentIds);
    ...
    public ResponseEntity<List<Integer>> getStudentPendingApprovals(int studentId) {
        try {
            Optional<List<Integer>> studentProjects = studentProjectDao
                    .findProjectIdByStudent_StudentIdAndStatus(studentId, Status.PENDING);
    ...
    public ResponseEntity<List<Integer>> getFacultiesByProject(int studentId, int projectId) {
        try {
            Optional<List<Integer>> studentProjects = studentProjectDao
                    .findProjectIdByStudent_StudentIdAndStatus(studentId, Status.PENDING);
            if (studentProjects.isPresent()) {
                List<Integer> projectIds = studentProjects.get().stream().filter(p -> !p.equals(projectId))
    ...
    public ResponseEntity<List<Integer>> findCompletedStudentIds(int projectId) {
        try {
            List<Integer> ids = studentProjectDao.findIdsCompletedStudentsByProjectId(projectId,
                    Arrays.asList(Status.COMPLETED, Status.APPROVED));
            return new ResponseEntity<>(ids, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
